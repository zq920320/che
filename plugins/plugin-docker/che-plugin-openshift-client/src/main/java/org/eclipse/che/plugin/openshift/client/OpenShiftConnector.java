...
public class OpenShiftConnector extends DockerConnector {
    ...
    private Pod getChePodByContainerId(String containerId) throws IOException {
        ...
        if (items.isEmpty()) {
            LOG.error("An OpenShift Pod with label {}={} could not be found", labelKey, containerId);
            throw new IOException("An OpenShift Pod with label " + labelKey + "=" + containerId +" could not be found");
        }

        if (items.size() > 1) {
            LOG.error("There are {} pod with label {}={} (just one was expeced)", items.size(), labelKey, containerId );
            throw new IOException("There are " + items.size() + " pod with label " + labelKey + "=" + containerId + " (just one was expeced)");
        }
